...
def run_transcription_batch(limit: int = 100, creator_username: str = None):
    """Process a batch of videos for a specific creator, falling back to others if none found."""
    # First, try to fetch videos for the specified creator
    query = (
        supabase
        .table("videos")
        .select("*")
    ...
    if creator_username:
        logger.info(f"Fetching up to {limit} videos for creator @{creator_username}")
        videos = query.eq("creator_username", creator_username).limit(limit).execute().data
    else:
        logger.info(f"Fetching up to {limit} videos with upload_status='done' and transcribe_status='pending'")
        videos = query.limit(limit).execute().data

    # If no videos found for the creator, fall back to any videos
    if not videos and creator_username:
        logger.info(f"No videos found for @{creator_username}, falling back to any creator")
        videos = query.limit(limit).execute().data
...
if __name__ == "__main__":
    run_transcription_batch(limit=2, creator_username="spaceiac")  # Specify creator for testing