...
BATCH_SIZE = 12  # Matches Gemini's high RPM
MAX_RETRIES = 3
SLEEP_INTERVAL = 1  # Faster with high RPM
...
def tag_videos_in_batch(limit: int = TEST_LIMIT):
    ...
    if failures:
        ...
        for video_id, _ in failures:
            ...
            try:
                result = process_video((video, 0, total_videos))  # idx not used in retry display
                if result[1]:
                    retry_successes.append(video_id)
                    print(f"[Retry] Tagged video {video_id} in {result[3]:.1f}s")
                else:
